...
number_suppliers = 10
number_retailers = 100
standard_distance = 0.02
min_lifetime = 4
...
gweights = [10.0, 0.0, 0.0, 10.0]
ptweights = [0.0, 10.0, 0.0, 10.0]
rtweights = [10.0, 10.0, 0.0, 10.0]
pweights = [0.0, 0.0, 1.0, 0.0]
...
## Start runs
for i in range(0, 4):
    gossip_weight = gweights[i]
    ptrust_weight = ptweights[i]
    rtrust_weight = rtweights[i]
    price_weight = pweights[i]
    if gossip_weight == 0.0:
        ...
        rbankruptcies = "Timestep, number \n 0, 0 \n"
        
        # lists to store strings
        patient_strings = [P_prices, P_qualities]
        retailer_strings = [R_prices, R_qualities]
        supplier_strings = [S_prices, S_qualities, S_invs]
        ...
        for step_number in range(1, total_steps+1):
            ...
            rbankruptcies += str(step_number)
            bankruptcies_this_step = 0
            ...
            for patient in patient_list:
                ...
                for retailer in range(0, number_retailers):
                    ...
                    retailer_values[retailer,1] = -price_weight*retailer_price[retailer] - patient_retailer_distance[patient,retailer] + ptrust_weight*patient_retailer_trust[patient, retailer]
            ...
            for retailer in retailer_list:
                ...
                if retailer_cash[retailer]>0:
                    ...
                    for supplier in range(0, number_suppliers):
                        supplier_values[supplier] = -price_weight*supplier_price[supplier] + rtrust_weight*retailer_supplier_trust[retailer,supplier]
            ...
            if step_number%(total_steps/10.0) == 0:
                print(str(systime.strftime("%H:%M:%S") + " - " + str(int(step_number*100/total_steps)) + "% of P" + str(price_weight) + "T" + str(ptrust_weight) + "G" + str(gossip_weight) + " complete"))
                
            rbankruptcies += "," + str(bankruptcies_this_step) + "\n"
        
        
            ## Make directory for output files
        directoryname = "P" + str(price_weight) + "T" + str(ptrust_weight) + "G" + str(gossip_weight)
        os.mkdir(directoryname)
        
        ## Write to files
        pfilenames = ["PatientPrices.csv", "PatientQualities.csv"]
        rfilenames = ["RetailerPrices.csv", "RetailerQualities.csv"]
        sfilenames = ["SupplierPrices.csv", "SupplierQualities.csv", "SupplierInventories.csv"]
        
        for l in range(0, len(pfilenames)):
            with open(directoryname + "/" + pfilenames[l], "w") as outfile:
                outfile.write(patient_strings[l])
        for n in range(0, len(rfilenames)):
            with open(directoryname + "/" + rfilenames[n], "w") as outfile:
                outfile.write(retailer_strings[n])
        for m in range(0, len(sfilenames)):
            with open(directoryname + "/" + sfilenames[m], "w") as outfile:
                outfile.write(supplier_strings[m])
                
        with open(directoryname + "/RetailerBankruptcies.csv", "w") as outfile:
            outfile.write(rbankruptcies) 
    else:
        for gossip_mode in g_modes:
            ...
            for step_number in range(1, total_steps+1):
                ...
                for patient in patient_list:
                    ...
                    if gossip_mode == "f":
                        ...
                        for retailer in range(0, number_retailers):
                            ...
                            # calculate retailer values
                            retailer_values[retailer,1] = -price_weight*retailer_price[retailer] - patient_retailer_distance[patient,retailer] + ptrust_weight*patient_retailer_trust[patient,retailer] + gossip_weight*gossip_trust[patient, retailer]
                        ...
                        for retailer in range(0,number_retailers):
                            ...
                            # calculate retailer values
                            retailer_values[retailer,1] = -price_weight*retailer_price[retailer] - patient_retailer_distance[patient,retailer] + ptrust_weight*patient_retailer_trust[patient, retailer] + gossip_weight*gossip_trust[retailer]
                ...
                rbankruptcies += str(step_number) + "," + str(bankruptcies_this_step) + "\n"
                
                ## Make directory for output files
            directoryname = "P" + str(price_weight) + "T" + str(ptrust_weight) + "G" + str(gossip_weight) + str(gossip_mode) 
            os.mkdir(directoryname)
